...
#: 东财 f100 二级名 → fetch_industry_list 一级名 (模块1 景气榜 prosperity_map 的键: 同花顺/东财一级 90 个)。**只给景气分查表用**
#: (run_pipeline.prosperity_lookup 第 ④ 步); 归属 / dom 分组 / 行业 PE 中位仍一律 f100 (老板 2026-09-23 拍板 ①, 这里不碰)。
#: 2026-09-24 卡 IND-PE 回修: 归属改成 f100 二级名后, 128 个 f100 名只有 67 个能靠「原名 / 去 Ⅱ/Ⅲ 后缀」查到景气分 (按票 4295/5573),
#: 房地产 (f100 = 房地产开发/房地产服务) / 建筑材料 (装修建材/水泥/玻璃玻纤) / 钢铁 (普钢/特钢Ⅱ/冶钢原料) / 美容护理 (化妆品/个护
#: 用品/医疗美容) 等 ~20 个一级行业的成分股整体查不到 → m4.cross_score 按 50 中性计 (w_prosperity 0.20), 高景气行业的票平白少 4-8 分。
#: 表怎么来的 (离线, 零联网): 09-18 东财直连快照 f100 × 一级成分缓存 (49 个一级行业 3408 只) 能对上的 11 个名按数据写 (每个名 100%
#: 落在同一个一级里: 房地产开发 80/80 → 房地产 …), 其余 43 个按申万 2021 二级 → 同花顺一级的对应关系写 (两套都是申万系分类)。
#: 键是 f100 原名 (含后缀), 查表先原名再去后缀; 值必须是一级名 (用例锁: 键 ⊆ EM_F100, 值 ⊆ EM_LEVEL1, 且键不能已经能直接查到)。
#: Tushare 兜底日 (快照行业列 = 别名表换过的东财口径) 同一张表也管用: 别名目标里的 水泥/玻璃玻纤/饮料乳品/饲料 由此查到。
F100_TO_L1 = {
    # 地产 / 建材 / 建筑 (前 5 个按 09-18 成分数据)
    "房地产开发": "房地产", "房地产服务": "房地产",
    "装修建材": "建筑材料", "水泥": "建筑材料", "玻璃玻纤": "建筑材料",
    "基础建设": "建筑装饰", "专业工程": "建筑装饰", "房屋建设Ⅱ": "建筑装饰", "装修装饰Ⅱ": "建筑装饰", "工程咨询服务Ⅱ": "建筑装饰",
    # 钢铁 (按数据) / 煤炭 / 石油
    "普钢": "钢铁", "特钢Ⅱ": "钢铁", "冶钢原料": "钢铁",
    "煤炭开采": "煤炭开采加工", "焦炭Ⅱ": "煤炭开采加工",
    "油气开采Ⅱ": "油气开采及服务", "油服工程": "油气开采及服务", "炼化及贸易": "石油加工贸易",
    # 美容护理 (按数据) / 食品饮料
    "化妆品": "美容护理", "个护用品": "美容护理", "医疗美容": "美容护理",
    "食品加工": "食品加工制造", "休闲食品": "食品加工制造", "调味发酵品Ⅱ": "食品加工制造",
    "饮料乳品": "饮料制造", "非白酒": "饮料制造",
    # 农林牧渔
    "种植业": "种植业与林业", "林业Ⅱ": "种植业与林业",
    # 渔业 / 饲料 / 动物保健Ⅱ 曾映射到 养殖业, 09-24 复核用同花顺「养殖业」成分缓存 (22 只) 反证: 这 35 只一只都不在里面,
    # 同花顺口径下它们不属养殖业 (饲料/动物保健/渔业 板块不在一级 90 名里), 按「宁可未知不借分」挪进 F100_L1_UNMAPPED。
    # 化工
    "塑料": "塑料制品", "橡胶": "橡胶制品",
    # 汽车
    "乘用车": "汽车整车", "商用车": "汽车整车", "汽车服务": "汽车服务及其他",
    # 军工
    "航空装备Ⅱ": "军工装备", "航天装备Ⅱ": "军工装备", "航海装备Ⅱ": "军工装备", "地面兵装Ⅱ": "军工装备",
    # 交运
    "航运港口": "港口航运", "航空机场": "机场航运", "铁路公路": "公路铁路运输",
    # 商贸 / 社服 / 传媒
    "一般零售": "零售", "专业连锁Ⅱ": "零售", "旅游零售Ⅱ": "零售",
    "旅游及景区": "旅游及酒店", "酒店餐饮": "旅游及酒店",
    "专业服务": "其他社会服务", "体育Ⅱ": "其他社会服务",
    "出版": "文化传媒", "广告营销": "文化传媒", "数字媒体": "文化传媒", "电视广播Ⅱ": "文化传媒",
}
#: 有意不映射 (f100 二级名在同花顺一级里没有唯一归宿: 其他家电/家电零部件/照明设备 在 白色/黑色/小家电/厨卫 之间, 农业综合 在
#: 种植/养殖 之间, 摩托车及其他 在 整车/零部件 之间, 文娱用品/饰品 在 家居用品/服装家纺/零售 之间): 宁可景气未知 (None → 50 中性,
#: 强左侧 tag 视为通过) 也不借别的行业的分。共 10 个名 ~143 只 (09-18 快照; 09-24 加 渔业/饲料/动物保健Ⅱ 35 只, 见上)。
#: 留在这里是让用例锁住「这些名不该硬映射」。
F100_L1_UNMAPPED = ("其他家电Ⅱ", "农业综合Ⅱ", "家电零部件Ⅱ", "摩托车及其他", "文娱用品", "照明设备Ⅱ", "饰品",
                    "渔业", "饲料", "动物保健Ⅱ")
